...
roach_video_raw = read.csv("", header=T)
...
roach_trials = read.csv("", header=T)
trial_info = read.csv("", header=T)
summary(trial_info) #there are some 2s for end.temp, checked lab book and they are typos
...
hist(residuals(roach_greg_m2)) #not acceptablegreat
...
text(x = -8, y = 73, labels = "A", cex = 2, font = 2 ) #note precise location will depend on the size of your plotting window
...
text(x = -2.8, y = 60, labels = "B", cex = 2, font = 2 ) #as above
...
roach_assocs_raw = read.csv("", header=T, stringsAsFactors = F)
...
prior = list(R = list(V = diag(c(1,0.0001),2,2), nu = 2, fix = 2), #residual variance is a 2x2 (co)variance matrix, with the variance of the2nd trait (strength) set to very small, as it does not vary within-individuals
